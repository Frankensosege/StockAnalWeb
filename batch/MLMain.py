...
from Utilities.comUtilities import get_property, get_time_str
from . import settings
from Utilities.LearnDataManager import load_data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default=get_time_str())
    ...
    base_dir = get_property('DATA', 'base_dir')
    output_json = get_property('DATA', 'jasonpath')
    output_model = get_property('DATA', 'modelpath')
    ...
    output_path = os.path.join(base_dir, output_json, output_name)
    if not os.path.isdir(output_path):
        os.makedirs(output_path)
    ...
    value_network_path = os.path.join(settings.BASE_DIR, output_model, value_network_name)
    policy_network_path = os.path.join(settings.BASE_DIR, output_model, policy_network_name)
    ...
    sl(__name__, output_name, 'ML').get_logger().info(params)
    # logger.info(params)
