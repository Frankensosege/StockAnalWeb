...
from Utilities.MLManager import Agent
from Utilities.TrainModel import load_data
...
class plotter:
    ...
    def __init__(self, plot_graph):
        ...
        self.sub_size = len(self.plot_graph)
        if self.sub_size <= 0:
            raise Exception('분석 지표가 선택되지 않았습니다.')

        self.sub_plts = plt.subplots(self.sub_size, 1)

    def draw_graphs(self, func, item_code, date_from=None, date_to=None, is_train=True):

        try:
            if self.sub_size != len(func):
                raise Exception('분석지표 함수목록과 graph의 수가 일치하지 않습니다.')
            for i in range(self.sub_size):
                func[i-1](self.sub_plts[i-1], item_code=item_code, date_from=date_from, date_to=date_to, is_train=is_train)
    ...
    def plot_candlestick_chart(self, ax, item_code, date_from, date_to, is_train):
        # df.set_index('date')
        # ax.clear()  # Clear the current plot in the subplot
        df = load_data(item_code, date_from=date_from, date_to=date_to, is_train=is_train)
    ...
    def plot_bollinger_bands(self, ax, item_code, date_from, date_to, is_train):
        df = load_data(item_code, date_from=date_from, date_to=date_to, is_train=is_train)
        xdate = df.date.astype('str')
        ax.plot(xdate, df['close_boll_high'], label="BBH", linewidth=0.7, color='k')
        ax.plot(xdate, df['close_boll_low'], label="BBL", linewidth=0.7, color='k')
        candlestick2_ohlc(ax, df['open'], df['high'], df['low'], df['close'], width=0.5, colorup='r', colordown='b')
