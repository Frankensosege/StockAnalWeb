...
def plot_candlestick_chart(ax, df):
    ax.clear()  # Clear the current plot in the subplot
    candlestick_ohlc(ax, df['Open'], df['High'], df['Low'], df['Close'], width=0.5, colorup='r', colordown='b')
...
class plotter:
    ...
    def draw_graphs(self, func, df):
        try:
            if self.sub_size != len(func):
                raise Exception('분석지표 함수목록과 graph의 수가 일치하지 않습니다.')
            for i in range(self.sub_size - 1):
                func[i](self.sub_plts[i], df)
